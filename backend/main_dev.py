...
# Development environment - use database_dev instead of database
from database_dev import get_db, create_tables, Transaction, Grant, BudgetItem, Allocation, FreeeToken, FreeeSync, Category
from schemas import (
...
from freee_csv_importer import FreeeCSVImporter
import subprocess
import signal
import psutil
...
freee_csv_importer = FreeeCSVImporter()
...
# Development environment root endpoint
@app.get("/")
async def root():
    return {"message": "NPO Budget Management System - Development Environment", "version": "1.0.0"}

@app.get("/health")
async def health_check():
    return {"status": "healthy", "environment": "development"}

@app.post("/api/freee/auto-sync/start")
def start_auto_sync():
    """freee自動同期を開始"""
    ...
    try:
        ...
        if scheduler_process and scheduler_process.poll() is None:
            return {"message": "freee自動同期は既に実行中です", "status": "running"}
        
        # スケジューラーを開始
        scheduler_process = subprocess.Popen(
            ["python", "freee_scheduler.py"],
            cwd="/home/tanaka/nagaiku-budget/backend",
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE
        )
        
        return {
            "message": "freee自動同期を開始しました",
            "status": "started",
            "pid": scheduler_process.pid
        }
    except Exception as e:
        return {"error": f"自動同期開始に失敗: {str(e)}", "status": "error"}

@app.post("/api/freee/auto-sync/stop")
def stop_auto_sync():
    """freee自動同期を停止"""
    global scheduler_process
    ...
    try:
        if not scheduler_process or scheduler_process.poll() is not None:
            return {"message": "freee自動同期は実行されていません", "status": "not_running"}
        ...
        return {
            "message": "freee自動同期を強制停止しました",
            "status": "force_stopped"
        }
    except Exception as e:
        return {"error": f"自動同期停止に失敗: {str(e)}", "status": "error"}

@app.get("/api/freee/auto-sync/status")
def get_auto_sync_status():
    ...
    if scheduler_process.poll() is None:
        ...
        return {
            "status": "stopped",
            "message": "自動同期は停止しています",
            "exit_code": scheduler_process.poll()
        }

@app.post("/api/freee/import-csv")
async def import_freee_csv(file: UploadFile = File(...)):
    """freee仕訳帳CSVをインポート"""
    try:
        # ファイル内容を読み取り
        ...
        # CSVを処理
        result = freee_csv_importer.process_freee_csv(file_content, 'freee_journal')
