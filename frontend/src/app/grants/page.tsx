...
const GrantsPage: React.FC = () => {
  ...
  const onBudgetCellValueChanged = async (params: any) => {
    try {
      ...
      if (isNewRow) {
        // グリッドの現在の状態を保存
        const gridState = budgetGridRef.current?.api.getState();
        
        // データを再読み込み
        await loadData();
        
        // グリッドの状態を復元（スクロール位置、フィルタなど）
        if (gridState && budgetGridRef.current?.api) {
          budgetGridRef.current.api.applyState(gridState);
        }
      }
                ...
                <>
                  <div className="flex justify-between items-start mb-2">
                    <div>
                      ...
                      <span className="text-xs text-gray-500">ID: {grant.id}</span>
                    </div>
